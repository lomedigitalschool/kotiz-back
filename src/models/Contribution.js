...
const { User, Contribution, Pull, Transaction } = db; // Ajout de User pour l'exhaustivité si nécessaire
...
    console.log('📨 Webhook de paiement reçu:', req.body);
    const signature = req.headers['x-semoa-signature'];
    // Assurez-vous que paymentService.processWebhook est implémenté pour valider la signature
    const webhookResult = await paymentService.processWebhook(req.body, signature);
